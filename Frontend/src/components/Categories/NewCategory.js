...
const NewCategory = (props) => {
  ...
  const [zNewCategory, setZNewCategory] = useState("z-40");
  ...
  const submitHandler = (event) => {
    ...
    categoryCtx.addItem(enteredName);
    setZNewCategory("z-20");
  };

  return (
    <Fragment>
      <div
        className={`absolute top-1/3 left-1/2 transform -translate-x-1/2  bg-gray-100 rounded-[8px] border border-solid border-grayBorder w-[600px] ${zNewCategory}`}
      >
        <RxCross1
          className="text-[20px] absolute top-[10px] right-[10px] text-blackText cursor-pointer"
          onClick={props.hideNewCategoryFunction}
        ></RxCross1>
      ...
      {categoryCtx.errorAddItem && (
        <PopUpError
          ...
          onAccept={props.hideNewCategoryFunction}

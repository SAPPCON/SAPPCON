...
const Name = ({ customerId }) => {
  ...
  const customerName = customer ? customer.name : "Nombre no encontrado";
  ...
  const submitHandler = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          responseData.error || "Error al actualizar el nombre del cliente"
        );
      }
        ...
        <div className="mx-auto my-0 flex  max-w-[600px] grow flex-col">
          ...
          <div className="flex flex-col rounded-[8px] border border-solid border-[#D5D9D9] px-[18px] py-[14px] font-sans">
            <div className="w-full text-[13px]">
              Si desea cambiar el nombre asociado al cliente seleccionado, puede

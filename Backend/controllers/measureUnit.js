  ...
  try {
    ...
    if (!measureUnit) {
      return res.status(404).json({
        ...
        messageinfo:
          "No se ha encontrado la unidad de medida con el id proporcionado.",
      });
    }
...
export const UpdateMeasureUnit = async (req, res) => {
  try {
    ...
    const userId = req.user.id;
    const updatedMeasureUnit = await MeasureUnit.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
  ...
  try {
    ...
    if (associatedServices.length > 0) {
      return res.status(400).json({
        message: "Unidad de medida no eliminada.",
        messageinfo:
          "No se puede eliminar la unidad de medida porque tiene servicios asociados.",
      });
    }

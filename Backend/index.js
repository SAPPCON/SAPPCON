...
async function init(port, dbHost, dbPort, dbName, dbUser, dbPass) {
  try {
    connectDB({ host: dbHost, port: dbPort, dbName: dbName, user:dbUser, pass:dbPass});
    if (isProduction) {
      const options = {
        key: fs.readFileSync("/etc/letsencrypt/live/sappcon.site/privkey.pem"), // Ruta clave privada
        cert: fs.readFileSync("/etc/letsencrypt/live/sappcon.site/fullchain.pem"), // Ruta certificado
      };
